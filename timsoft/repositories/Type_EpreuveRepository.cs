...
namespace timsoft.repositories
{
    public class Type_EpreuveRepository : IType_EpreuveRepository
    {
        ...
        public Type_Epreuve AddType_Epreuve(string type_Epreuve)
        {
            Type_Epreuve Te = new Type_Epreuve();
            Te.Type = type_Epreuve;
            _context.Add(Te);
        ...
        public string UpdateType_Epreuve(string type_Epreuve, int id)
        {
            var itemToUpdate = _context.Type_Epreuve.Where(i => i.IdType == id).FirstOrDefault();
            if (itemToUpdate == null)
                throw new NullReferenceException();

            itemToUpdate.Type = type_Epreuve;
            _context.SaveChanges();
